/*
  # Undo Recent Subscription Migrations

  1. Changes Made
    - Remove subscription change triggers and notifications
    - Remove real-time publication for subscriptions
    - Drop additional indexes that were added in recent migrations
    - Restore original user_subscriptions view
    - Keep essential RLS policies and core indexes

  2. Security
    - Maintain existing RLS policies for proper access control
    - Keep service role permissions for webhook processing
*/
...
-- Ensure the updated_at trigger is still present (using proper syntax)
DO $$
BEGIN
...
EXCEPTION
    WHEN others THEN
        -- If there's any error, just ignore it (trigger might already exist)
        NULL;
END $$;